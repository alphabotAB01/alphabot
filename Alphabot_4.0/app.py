...
def login():
    ...
    if request.method == 'POST':
        ...
        if completion == False:
            ...
            return redirect(url_for('controllaAlphaboz'))
...
def controllaAlphaboz():
    if request.method == 'POST':
        ...
        memorizza_azione(comando_ricevuto)
...
"""Questa funzione memeorizza il login al database dell'utente"""
...
def memorizza_azione(azione):
    now_date = datetime.now().strftime("%d/%m/%Y")
    now_time = datetime.now().strftime("%H:%M:%S")
    con = sqlite3.connect("./alphabot.db")
    cur = con.cursor()
    cur.execute(f"INSERT INTO registro (azione, data, ora)" \
                f"VALUES ('{azione}' '{now_date}', '{now_time}')")
    con.commit()
    cur.close()
"""-----------------------------------------------------------"""
