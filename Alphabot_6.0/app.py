...
@app.route('/api/v1/sensor/obstacles', methods=['GET'])
def api_statoIR():
    return jsonify({"Left": alphabot.sensoreIR_sx(), "Right": alphabot.sensoreIR_dx()})


@app.route('/api/v1/motors/left', methods=['GET'])
def setPWM_left():
    try:
        messaggio_ritorno = PROCEDURA_RIUSCITA
        if 'pwm' in request.args:
            if 'time' in request.args:
                alphabot.set_motor(-int(request.args['pwm']), int(request.args['pwm']),
                                   durata_comando=int(request.args['time']))
            else:
                alphabot.set_motor(-int(request.args['pwm']), int(request.args['pwmR']))
    except:
        messaggio_ritorno = PROCEDURA_FALLITA
    return messaggio_ritorno
...
def setPWM_both():
    try:
        messaggio_ritorno=PROCEDURA_RIUSCITA
        if 'pwmR' in request.args and 'pwmL' in request.args:
            if 'time' in request.args:
                alphabot.set_motor(int(request.args['pwmL']), int(request.args['pwmR']), durata_comando=int(request.args['time']))
            else: alphabot.set_motor(int(request.args['pwmL']), int(request.args['pwmR']))
    except:
        messaggio_ritorno=PROCEDURA_FALLITA
    return messaggio_ritorno


@app.route(f'/comando', methods=['GET', 'POST'])
